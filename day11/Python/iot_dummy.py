...
for day in range(total_days):   # 10000일 반복
    ...
    for i in range(records_per_day):  # 100번 반복
        ...
        temp = round(random.uniform(19.0, 28.0), 1)  # 19~28도 사이의 온도를 랜덤하고, 소수점 첫번째에서 반올림
        humid = round(random.uniform(30.0, 60.0), 2)    # 30%~60% 사이 습도 랜덤, 소수점 첫번째 반올림
        batch.append((timestamp, 'DINNING', temp, humid))

        if len(batch) >= batch_size:
            cursor.executemany(insert_query, batch)  # 한꺼번에 배치 사이즈만큼 데이터 삽입
            conn.commit()
            batch.clear()
