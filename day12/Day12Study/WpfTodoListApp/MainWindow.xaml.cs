...
public partial class MainWindow : MetroWindow
{
    ...
    private async Task GetDatas()
    {
        ...
        try  // API 호출
        {
            ...
            todoItems.CopyForm(items);  // ObservableCollection으로 형변환
    ...
    // async 시 Task가 리턴값이지만 버튼클릭 이벤트 메서드와 연결시는 Task -> void로 변경해줘야 연결 가능
    ...
    private async void GrdTodoItems_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
    {
        try
        {
            ...
            var Id = (GrdTodoItems.SelectedItem as TodoItem)?.Id;   // ?. -> Null이 생겨도 예외 발생 안함
